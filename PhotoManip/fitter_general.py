    ...
    if already_run_calc_all_chis != True:
        if dilution_factor==0:
            ...
            for i, line in enumerate(samples):
                ...
                if ndim==1:
                    my_objective = class_chi2.objective_with_uncertainties(model_nparam(line,data[:, 0]).model_array(), data, sigmas=uncertainties)
                elif ndim==2:
...
def plot_1D_marginalized_distribution(flatchain_path, bests=None,output_pdf_file_path='.',output_txt_file_path='.', parameters_labels=None,number_bins=None):
        ...
        if sample.ndim > 1:
            ...
            pylab.figure()
            n, bin, patches = pylab.hist(sample[:], bins=number_bins, alpha=0.5, color='orange')
            #               pylab.axvline(bn_mcmc2, color='r', linestyle='dashed', linewidth=3,label=r'$\chi^2={0}$'.format(maxparam[1,np.shape(maxparam)[1]-1]))
            #               pylab.axvline(bn_mcmc3, color='b', linestyle='dashed', linewidth=3,label=r'$\chi^2={0}$'.format(maxparam[2,np.shape(maxparam)[1]-1]))
            #               pylab.legend()
            bin2 = np.zeros((np.shape(n)))
            ...
            for i, b in enumerate(bin):
                if float(sum(n[:i])) / sum(n) >= 0.16:
                    ...
                    nlower = bin[i]
                    #print(
                    #'at the {0}th bin, i.e. at b={1}, the ration float(sum(n[:i]))/sum(n) is {2} and the 16percent limit has been reached'.format(
                    #    i, bin2[i], float(sum(n[:i])) / sum(n)))
                    break
            for i, b in enumerate(bin):
                if float(sum(n[:i])) / sum(n) >= 0.84:
                    ...
                    # print('sun(n[:i]) is',sum(n[:i])
                    #print(
                    #'at the {0}th bin, i.e. at b={1}, the ration float(sum(n[:i]))/sum(n) is {2} and the 16percent upper limit has been reached'.format(
                    #    i, bin2[i], float(sum(n[:i])) / sum(n)))
                    break
            for i, b in enumerate(bin):
                if float(sum(n[:i])) / sum(n) >= 0.5:
                    ...
                    # print('sun(n[:i]) is',sum(n[:i])
                    #print(
                    #'at the {0}th bin, i.e. at b={1}, the ration float(sum(n[:i]))/sum(n) is {2} and the 50percent upper limit has been reached'.format(
                    #    i, bin2[i], float(sum(n[:i])) / sum(n)))
                    break
            pylab.plot(bin2, n, color='k')
            if bests is not None:
                pylab.axvline(bests, color='k', linestyle='dashed', linewidth=3,
                              label=r'Best fit')  # . $\chi^2={0}$'.format(maxparam[0, np.shape(maxparam)[1] - 1]))
            pylab.axvline(nlower, color='b', linestyle='dashed', linewidth=3,
                          label=r'$1\sigma$-range lower limit')
            pylab.axvline(nmedian, color='c', linestyle='dashed', linewidth=3, label=r'median')
            pylab.axvline(nupper, color='r', linestyle='dashed', linewidth=3,
                          label=r'$1\sigma$-range upper limit')
            pylab.legend(fontsize='x-small')
            pylab.ylabel('Marginalized Posterior Distribution')
            if parameters_labels != None:
                ...
                pylab.savefig(output_pdf_file_path + '/histo_param_' + str(0) + '.pdf', facecolor='w',
                              ...
                              pad_inches=0.1)
            if bests is None:
                sigma[:] = nlower, nupper, float(nupper - nlower) / 2, nmedian
                np.savetxt(output_txt_file_path + '/1sigma.txt', sigma,
                           header='lower 1sigma, upper 1sigma, diff/2,median')
            else:
                sigma[:] = nlower, nupper, float(nupper - nlower) / 2, nmedian, bests
                np.savetxt(output_txt_file_path + '/1sigma.txt', sigma,
                           header='lower 1sigma, upper 1sigma, diff/2,median, best-fit')
